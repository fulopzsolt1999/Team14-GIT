...
var main = function () {
    var eredmenyekInterface = getInterfaceFromArray();
    var teamScore = getTeamScore(eredmenyekInterface);
    var isTie = checkIfTie(eredmenyekInterface);
    var teamNameOfBarcelona = getTeamNameOfBarcelona(eredmenyekInterface);
    var matchesOnSpecificDate = getMatchesOnSpecificDate(eredmenyekInterface);
    var matchesOnStadionsCount = getMatchesOnStadionsCount(eredmenyekInterface);
    logAllResults(teamScore, isTie, teamNameOfBarcelona, matchesOnSpecificDate, matchesOnStadionsCount);
};
main();
function getInterfaceFromArray() {
    var eredmenyekInterface = [];
    for (var i = 0; i < eredmenyek.length; i++) {
        eredmenyekInterface.push({
            hazai: eredmenyek[i].split(";")[0],
            idegen: eredmenyek[i].split(";")[1],
            hazai_pont: Number(eredmenyek[i].split(";")[2]),
            idegen_pont: Number(eredmenyek[i].split(";")[3]),
            helyszin: eredmenyek[i].split(";")[4],
            idopont: eredmenyek[i].split(";")[5],
        });
    }
    return eredmenyekInterface;
}
function getTeamScore(eIF) {
    var getTeamFromUser = prompt("Adja meg a keresett csapat nevét.");
    var hazaiCounter = 0;
    var idegenCounter = 0;
    for (var _i = 0, eIF_1 = eIF; _i < eIF_1.length; _i++) {
        var match = eIF_1[_i];
        if (match.hazai === getTeamFromUser) {
            hazaiCounter++;
        }
        else if (match.idegen === getTeamFromUser) {
            idegenCounter++;
        }
    }
    if (hazaiCounter != 0 || idegenCounter != 0) {
        return [getTeamFromUser, hazaiCounter, idegenCounter];
    }
    return null;
}
function checkIfTie(eIF) {
    for (var _i = 0, eIF_2 = eIF; _i < eIF_2.length; _i++) {
        var match = eIF_2[_i];
        if (match.hazai_pont === match.idegen_pont) {
            return true;
        }
        return false;
    }
}
function getTeamNameOfBarcelona(eIF) {
    for (var _i = 0, eIF_3 = eIF; _i < eIF_3.length; _i++) {
        var match = eIF_3[_i];
        var teamName = match.hazai.split(" ");
        for (var _a = 0, teamName_1 = teamName; _a < teamName_1.length; _a++) {
            var name_1 = teamName_1[_a];
            if (name_1 === "Barcelona") {
                return match.hazai;
            }
        }
    }
    return "Nincs ilyen team.";
}
function getMatchesOnSpecificDate(eIF) {
    var results = [];
    for (var _i = 0, eIF_4 = eIF; _i < eIF_4.length; _i++) {
        var match = eIF_4[_i];
        if (match.idopont === "2004-11-21") {
            results.push(match);
        }
    }
    return results;
}
function getMatchesOnStadionsCount(eIF) {
    var stadiumCounter = [];
    var allStadiums = [];
    for (var _i = 0, eIF_5 = eIF; _i < eIF_5.length; _i++) {
        var match = eIF_5[_i];
        if (!allStadiums.includes(match.helyszin)) {
            allStadiums.push(match.helyszin);
            stadiumCounter.push({ name: match.helyszin, count: 0 });
        }
    }
    for (var i = 0; i < stadiumCounter.length; i++) {
        for (var _a = 0, eIF_6 = eIF; _a < eIF_6.length; _a++) {
            var match = eIF_6[_a];
            if (match.helyszin === stadiumCounter[i].name) {
                stadiumCounter[i].count += 1;
            }
        }
    }
    return stadiumCounter.filter(function (i) { return i.count >= 20; });
}
function logAllResults(teamScore, isTie, teamNameOfBarcelona, matchesOnSpecificDate, matchesOnStadionsCount) {
    if (teamScore != null) {
        console.log("3. Feladat:\n\t".concat(teamScore[0], ": Hazai: ").concat(teamScore[1], ", Idegen: ").concat(teamScore[2]));
    }
    else {
        console.log("Nincs ilyen csapat.");
    }
    console.log("4. Feladat:\n\tVolt d\u00F6ntetlen? ".concat(isTie ? "Igen." : "Nem."));
    console.log("5. Feladat:\n\tBarcelonai csapat neve: ".concat(teamNameOfBarcelona));
    console.log("6. Feladat:");
    for (var _i = 0, matchesOnSpecificDate_1 = matchesOnSpecificDate; _i < matchesOnSpecificDate_1.length; _i++) {
        var match = matchesOnSpecificDate_1[_i];
        console.log("\t".concat(match.hazai, "-").concat(match.idegen, " (").concat(match.hazai_pont, ": ").concat(match.idegen_pont, ")"));
    }
    console.log("7. Feladat:");
    for (var _a = 0, matchesOnStadionsCount_1 = matchesOnStadionsCount; _a < matchesOnStadionsCount_1.length; _a++) {
        var stadium = matchesOnStadionsCount_1[_a];
        console.log("\t".concat(stadium.name, ": ").concat(stadium.count));
    }
}
