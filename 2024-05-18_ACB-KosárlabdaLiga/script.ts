...
const main = (): void => {
   const eredmenyekInterface: ACBLeague[] = getInterfaceFromArray();
   const teamScore: [string | null, number, number] | null = getTeamScore(eredmenyekInterface);
   const isTie: boolean | undefined = checkIfTie(eredmenyekInterface);
   const teamNameOfBarcelona: string | undefined = getTeamNameOfBarcelona(eredmenyekInterface);
   const matchesOnSpecificDate: ACBLeague[] = getMatchesOnSpecificDate(eredmenyekInterface);
   const matchesOnStadionsCount: StadiumCount[] = getMatchesOnStadionsCount(eredmenyekInterface);
   logAllResults(
      teamScore,
      isTie,
      teamNameOfBarcelona,
      matchesOnSpecificDate,
      matchesOnStadionsCount
   );
};
main();

function getInterfaceFromArray(): ACBLeague[] {
   var eredmenyekInterface: ACBLeague[] = [];
...
function getTeamScore(eIF: ACBLeague[]): [string | null, number, number] | null {
   var getTeamFromUser: string | null = prompt("Adja meg a keresett csapat nevét.");
   var hazaiCounter: number = 0;
   var idegenCounter: number = 0;

   for (const match of eIF) {
...
function checkIfTie(eIF: ACBLeague[]): boolean | undefined {
   for (const match of eIF) {
...
function getTeamNameOfBarcelona(eIF: ACBLeague[]): string | undefined {
   for (const match of eIF) {
      var teamName: string[] = match.hazai.split(" ");
...
function getMatchesOnSpecificDate(eIF: ACBLeague[]): ACBLeague[] {
   var results: ACBLeague[] = [];
   for (const match of eIF) {
...
function getMatchesOnStadionsCount(eIF: ACBLeague[]): StadiumCount[] {
   var stadiumCounter: StadiumCount[] = [];
   var allStadiums: string[] = [];
   for (const match of eIF) {
   ...
   for (let i = 0; i < stadiumCounter.length; i++) {
      for (const match of eIF) {
   ...
   return stadiumCounter.filter((i) => i.count >= 20);
}

function logAllResults(
   teamScore: [string | null, number, number] | null,
   isTie: boolean | undefined,
   teamNameOfBarcelona: string | undefined,
   matchesOnSpecificDate: ACBLeague[],
   matchesOnStadionsCount: StadiumCount[]
): void {
   if (teamScore != null) {
      console.log(
         `3. Feladat:\n\t${teamScore[0]}: Hazai: ${teamScore[1]}, Idegen: ${teamScore[2]}`
      );
   } else {
      console.log("Nincs ilyen csapat.");
   }
   console.log(`4. Feladat:\n\tVolt döntetlen? ${isTie ? "Igen." : "Nem."}`);
   console.log(`5. Feladat:\n\tBarcelonai csapat neve: ${teamNameOfBarcelona}`);
   console.log("6. Feladat:");
   for (const match of matchesOnSpecificDate) {
      console.log(`\t${match.hazai}-${match.idegen} (${match.hazai_pont}: ${match.idegen_pont})`);
   }
   console.log("7. Feladat:");
   for (const stadium of matchesOnStadionsCount) {
      console.log(`\t${stadium.name}: ${stadium.count}`);
   }
}
